...
AUTORUN TASK kernel void pe_kernel( void ) {
  ...
  do {
    ...
    #pragma unroll
    for (int fw_inc = 0; fw_inc < FW_VECTOR; fw_inc++) {
      filter.v[fw_inc] = filter_cache[filter_read_addr][n_inc][fw_inc];
    }
   
    BiasBnParam bias_bn = bias_bn_cache[filter_bias_read_page][n_inc];
    ...
    for (int n = 0; n < N_VECTOR; n++) {
      if (n == n_inc) {
        #pragma unroll
        for (int w_inc = 0; w_inc < W_VECTOR; w_inc++) {
          Mreal sum = conv_start ? bias_bn.bias : result[n][w_inc];
          result[n][w_inc] = sum + dot_sum_fw_vec[w_inc];
        }
      }
    }
      ...
      for (int n = 0; n < N_VECTOR; n++) {
        if (n == n_inc) {
          ...
          for (int w_inc = 0; w_inc < W_VECTOR; w_inc++) {
            ...
            long int bn_alpha_inflat = (long int) (result[n][w_inc] * bias_bn.alpha);
            ...
            if (n_inc == PRINT_N && cycle >= debug_cycle && cycle < debug_cycle + debug_range) 
              printf("PE cycle=%d/%d w_inc=%d result=%d bias_bn.alpha=%d bias_bn.beta=%d pe_output.data.v=%d\n", cycle, cycle_end, w_inc, result[n][w_inc], bias_bn.alpha, bias_bn.beta, pe_output.data.v[w_inc]);
#endif
          }

          write_channel_intel(pe_output_channel[n], pe_output);
        }
      }
    }
