...
bool NetWork::init_network()
{
  ...
  const int filter_raw_size =  NUM_CONVOLUTIONS * MAX_FILTER_SIZE * NEXT_POWER_OF_2(S_VECTOR * C_VECTOR);
  ...
  bias_bn = (bias_bn_param_t*)alignedMalloc(sizeof(bias_bn_param_t) * bias_bn_size);
  if (bias_bn == NULL) ERROR("Cannot allocate enough space for bias_bn\n");
  memset(bias_bn, 0, sizeof(bias_bn_param_t) * bias_bn_size);
  ...
  //load_model(model_file, filter_raw, bias,alpha,beta, q);
  load_model(model_file, filter_raw, bias_bn, q);

  const int filter_device_size = NUM_CONVOLUTIONS * MAX_FILTER_SIZE * NEXT_POWER_OF_2(S_VECTOR * C_VECTOR);
  ...
  filter_convert(filter, filter_raw, filter_real);
}

bool NetWork::init_buffer()
{ 
  // output array
  ...
  // bias_bn buffer
  const int bias_bn_size = NUM_CONVOLUTIONS * MAX_BIAS_SIZE;
  bias_bn_buffer = clCreateBuffer(platform.context, CL_MEM_READ_ONLY, sizeof(bias_bn_param_t) * bias_bn_size, NULL, &status);
  checkError(status,"Failed clCreateBuffer : bias_bn_buffer");
  status = clEnqueueWriteBuffer(platform.bias_bn_queue, bias_bn_buffer, CL_TRUE, 0, sizeof(bias_bn_param_t) * bias_bn_size, bias_bn, 0, NULL, NULL);
  ...
  status = clEnqueueWriteBuffer(platform.wait_after_conv_queue, wait_after_conv_cycles, CL_TRUE, 0, sizeof(int) * NUM_CONVOLUTIONS, &wait_after_conv[0], 0, NULL, NULL);
  checkError(status,"Failed clEnqueueWriteBuffer : wait_after_conv");
