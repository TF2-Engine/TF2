...
void Runner::enqueue_kernels(bool create_events, bool enqueue_infinite_loop) {
...
void Runner::wait_for_all_kernels() {
  for (int i = 0; i < platform.kernels.size(); i++) {
...
void Runner::release_all_events() {
  for (int i = 0; i < platform.kernels.size(); i++) {
...
void Runner::run()
{
  ...
  // sequencer
  kernel_info_t sequencer = platform.find_kernel("sequencer");
  ...
  // retriever
  kernel_info_t feature_reader = platform.find_kernel("retriever");
  ...
  // filter reader
  kernel_info_t filter_reader = platform.find_kernel("filter_reader");
  ...
  // relu
  kernel_info_t relu = platform.find_kernel("relu");
  ...
  // pool
  kernel_info_t pool = platform.find_kernel("pool");
  ...
  // pool_tail
  kernel_info_t pool_tail = platform.find_kernel("pool_tail");
  ...
  // end_pool
  kernel_info_t end_pool = platform.find_kernel("end_pool");

  arg_idx = 0;

  status = clSetKernelArg(end_pool.kernel, arg_idx++, sizeof(cl_int), (void*)&num_images);
  checkError(status,"end_pool_kernel: Failed set arg %d.", arg_idx - 1);

  // feature_writer
  kernel_info_t feature_writer = platform.find_kernel("feature_writer");
  ...
  // enqueue kernels
  enqueue_kernels(/* create_events */ false, /* enqueue_infinite_loop */ true);
  int torank[2]={0};
  ...
  arg_idx=0;
  kernel_info_t input_reader = platform.find_kernel("input_reader");
  ...
  enqueue_kernels(/* create_events */ true, /* enqueue_infinite_loop */ false);
