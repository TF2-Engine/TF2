...
template<typename T>
void filter_trans(T *filter_input, T *Transform_input) {
  T filter_media[CHAS * 2][FILTER_FIRST][FILTER_FIRST];

  for (int i = 0; i < CHAS * 2; i++) {
    for (int j = 0; j < FILTER_FIRST; j++) {
      for (int k = 0; k < FILTER_FIRST; k++) {
        // filter_media[i][j][k] = 0x40;
        filter_media[i][j][k] = 0b0000;
      }
    }
  }

  T filter_width[CHAS * 3][FILTER_FIRST][FILTER_FIRST];

  // order is channel, heights, width
  for (int i = 0; i < CHAS * 3; i++) {
    for (int j = 0; j < FILTER_FIRST; j++) {
      for (int k = 0; k < FILTER_FIRST; k++) {
        filter_width[i][j][k] = 0b0000;
      }
    }
  }
  ...
  T filter_heights[6*CHAS][FILTER_FIRST][FILTER_FIRST];
...
// filter_raw[N][C][H][W]
void LoadModel(char *filename, int8 *filter_raw, BiasBnParam *bias_bn, float (*scale)[2]) {
  FILE* infile;
  ...
  for (int layer = 0; layer < NUM_CONVOLUTIONS; layer++) {
      ...
      for (int n = 0; n < N; n++) {
        for (int c = 0; c < C; c++) {
          ...
          for (int h = 0; h < H; h++) {
            for (int w = 0; w < W; w++) {
              float filter_tems;
              // fread(&filter_tems, sizeof(int4), 1, infile);
              // fread(&filter_tems, sizeof(int4), 1, infile);
              fread(&filter_tems, sizeof(float), 1, infile);
              
              if (layer >= DEVICE_START_LAYER && layer < DEVICE_END_LAYER) {
                char filter_char = filter_tems;
                if (BIT_IS_SET(filter_char, 7)) {
                  filter_char = ((0x8f & filter_char)^(0x0f)) + 1;
                } 
              
                // filter_raw[filter_addr_offset + n * C * H * W + c * H * W + h * W + w] = Get_real(filter_tems, expand);
                filter_raw[filter_addr_offset + n * C * H * W + c * H * W + h * W + w].char_value = filter_char;
              } else {
                filter_raw[filter_addr_offset + n * C * H * W + c * H * W + h * W + w].char_value = filter_tems;
              }
              #ifdef PRINT_H_FILTER_INPUT
                if(layer ==  PRINT_LAYER - 1)
                printf("Filter Raw layer=%d n=%d c=%d h=%d w=%d filter_raw=%f\n", layer ,n, c, h, w, filter_tems, filter_raw[filter_addr_offset + n * C * H * W + c * H * W + h * W + w]);
              #endif
            }
          }
        }
        ...
        // fread( &scale[0], sizeof(float), 1, infile1 );
        // fread( &scale[1], sizeof(float), 1, infile1 );
      // }
      ...
      if (kBiasEnable[layer]) {				  
        for (int n = 0; n < N; n++) {
          ...
          // float bias_trans_coe = 1 << (INFLAT - q_fixed_gap);
  	      float bias_data;
          // fread( &bias_data, sizeof(int), 1, infile );
          fread( &bias_data, sizeof(float), 1, infile );
  	      bias_bn[bias_addr_offset + n].bias = bias_data;    
        }
      } else {
        for (int n = 0; n < N; n++) {
    ...
    for (int n = 0; n < N; n++) {
      if (layer >= DEVICE_START_LAYER && layer < DEVICE_END_LAYER) {
        ...
        float scale_pre =  scale[pre_layer][1];
        //bias_bn[bias_addr_offset + n].scale[0] = 1 / scale[layer][0]; 
        bias_bn[bias_addr_offset + n].scale[1] = scale[layer][1] / scale_pre; 
        // bias_bn[bias_addr_offset + n].scale[2] = scale[layer][2]; 
        // bias_bn[bias_addr_offset + n].alpha = alpha[n] * scale[n][1];
        // bias_bn[bias_addr_offset + n].beta = beta[n] > 0 ? (scale[n][1] * beta[n] + 0.5) : (scale[n][1] * beta[n] - 0.5);
        bias_bn[bias_addr_offset + n].alpha = (!kIpoolEnable[layer]) ? alpha[n] / scale[layer][0] * scale[layer][1] : 1.0;
        bias_bn[bias_addr_offset + n].beta = (!kIpoolEnable[layer]) ? beta[n] * scale[layer][1] : 1.0;
        // bias_bn[bias_addr_offset + n].alpha = alpha[n];
        // bias_bn[bias_addr_offset + n].beta = beta[n];
      } else {
        bias_bn[bias_addr_offset + n].scale[0] = 1 / scale[layer][0]; 
        bias_bn[bias_addr_offset + n].scale[1] = scale[layer][1]; 
        bias_bn[bias_addr_offset + n].alpha = (!kIpoolEnable[layer]) ? alpha[n] : 1.0;
        bias_bn[bias_addr_offset + n].beta = (!kIpoolEnable[layer]) ? beta[n] : 1.0;
      }
      #ifdef PRINT_H_BN_INPUT
        if(layer ==  PRINT_LAYER - 1)
          printf("Alpha_Beta layer=%d n=%d bias_addr_offset=%d alpha=%f beta=%f scale_0=%f scale_1=%f bias=%d\n", layer, n, bias_addr_offset, bias_bn[bias_addr_offset + n].alpha, bias_bn[bias_addr_offset + n].beta,  bias_bn[bias_addr_offset + n].scale[0], bias_bn[bias_addr_offset + n].scale[1], bias_bn[bias_addr_offset + n].bias);
      #endif 
    }
  // }
  ...
  // int4 *first_layer_filter_trans = (int4*)malloc(sizeof(real) * Trans_size);
  // memset(first_layer_filter_trans, 0, sizeof(int4)*64*9*27);
...
// void FilterConvert(real *filter, real *filter_raw, real *filter_real) {
  void FilterConvert(real *filter, int8 *filter_raw, real *filter_real) {
              ...
              for (int fw_inc = 0; fw_inc < FW_VECTOR; fw_inc++) {
                for (int c_inc = 0; c_inc < C_VECTOR; c_inc++) {
                  ...
                  if (not_out_of_bounds) {
                    filter_real[addr] = reinterpret_cast<real>(filter_raw[filter_raw_addr].char_value);
                    #ifdef PRINT_H_FILTER_REAL
                    // printf("Filter Converter raw_addr=%d addr=%d s=%d filter_real_val=%d\n", filter_raw_addr, addr, filter_real[addr] & 0x80, filter_real[addr] & 0x7f);
                    printf("Filter Converter raw_addr=%d addr=%d filter_real=%d filter_raw=%d\n", filter_raw_addr, addr, filter_real[addr], filter_raw[filter_raw_addr].char_value);
                    #endif
                  }
                }
              }
